...
using static TownOfUs.CustomOption.CustomOption;
...
namespace TownOfUs.CustomOption {
    public class CustomOption {
        ...
        public static void switchPreset(int newPreset) {
            ...
            vanillaSettings = TownOfUs.Instance.Config.Bind($"Preset{preset}", "GameOptions", "");
            loadVanillaOptions();
        ...
        public static bool loadVanillaOptions() {
            ...
            if (gameOptions.Version < 8) {
                TownOfUs.Logger.LogMessage("tried to paste old settings, not doing this!");
                return false;
            } 
        ...
        public static int deserializeOptions(byte[] inputValues) {
            ...
            while (reader.BaseStream.Position < inputValues.Length) {
                try {
                    ...
                    TownOfUs.Logger.LogWarning($"id:{lastId}:{e}: while deserializing - tried to paste invalid settings!");
                    errors++;
                }
            }
        ...
        public static void copyToClipboard() {
            GUIUtility.systemCopyBuffer = $"{TownOfUs.VersionString}!{Convert.ToBase64String(serializeOptions())}!{vanillaSettings.Value}";
        }
        ...
        public static int pasteFromClipboard() {
            ...
            try {
                ...
                ShareOptionSelections();
                if (TownOfUs.Version > versionInfo && versionInfo < System.Version.Parse("5.1.0")) {
                    vanillaOptionsFine = false;
                    DestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "Host Info: Pasting vanilla settings failed, TOR Options applied!");
                } else {
                    vanillaSettings.Value = vanillaSettingsSub;
                    vanillaOptionsFine = loadVanillaOptions();
                }
            } catch (Exception e) {
                TownOfUs.Logger.LogWarning($"{e}: tried to paste invalid settings!\n{allSettings}");
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType) {
            ...
            foreach (var option in relevantOptions) {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type) {
                    ...
                    if ((int)optionType == 99)
                        categoryHeaderMasked.Title.text = new Dictionary<CustomOptionType, string>() { { CustomOptionType.Crewmate, "Crewmate Roles" }, { CustomOptionType.Neutral, "Neutral Roles" }, 
    ...
    class GameOptionsMenuStartPatch {
        ...
        public static void Postfix(GameSettingMenu __instance) {
            ...
            bgrenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.CopyPasteBG.png", 175f);
            ...
            copyButtonRenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.Copy.png", 100f);
            copyButton.transform.GetChild(1).transform.localPosition = Vector3.zero;
            copyButtonActiveRenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.CopyActive.png", 100f);
            ...
            pasteButtonRenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.Paste.png", 100f);
            pasteButtonActiveRenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.PasteActive.png", 100f);
            ...
            foreach (CustomOption option in options) {
                if (option.parent != null) {
                    ...
                    if (option == CustomOptionHolder.NonKillingNeutralRolesCountMin) {
                        ...
                        var optionValue = (min == max) ? $"{max}" : $"{min} - {max}";
                        sb.AppendLine($"{optionName}: {optionValue}");
                    } else if ((option == CustomOptionHolder.NonKillingNeutralRolesCountMax) || (option == CustomOptionHolder.KillingNeutralRolesCountMax)) {
                        continue;
                    } else {
    ...
    public static class GameOptionsNextPagePatch
    {
        public static void Postfix(KeyboardJoystick __instance)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Tab)) {
                TownOfUs.optionsPage = (TownOfUs.optionsPage + 1) % 7;
            }
            ...
            if (Input.GetKeyDown(KeyCode.Alpha7) || Input.GetKeyDown(KeyCode.Keypad7)) {
                TownOfUs.optionsPage = 6;
            }
            if (Input.GetKeyDown(KeyCode.F1))
                HudManagerUpdate.ToggleSettings(HudManager.Instance);
            if (TownOfUs.optionsPage >= GameOptionsDataPatch.maxPage) TownOfUs.optionsPage = 0;
        }
    }
    ...
    public class HudManagerUpdate {
        ...
        public static void Postfix(HudManager __instance) {
            ...
            if (!toggleSettingsButton || !toggleSettingsButtonObject) {
                ...
                renderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.Settings_Button.png", 100f);
                rendererActive.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.Settings_ButtonActive.png", 100);
            ...
            if (!toggleZoomButton || !toggleZoomButtonObject) {
                ...
                tZrenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.Minus_Button.png", 100f);
                tZArenderer.sprite = Utils.loadSpriteFromResources("TownOfUs.Resources.Minus_ButtonActive.png", 100);
