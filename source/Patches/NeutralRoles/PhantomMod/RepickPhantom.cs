...
namespace TownOfUs.NeutralRoles.PhantomMod
{
    ...
    public class RepickPhantom
    {
        private static void Postfix(HudManager __instance)
        {
            ...
            {
                var toChooseFromAlive = PlayerControl.AllPlayerControls.ToArray().Where(x => (x.Is(Faction.NeutralKilling) || x.Is(Faction.NeutralEvil) || x.Is(Faction.NeutralBenign)) && !x.Is(ModifierEnum.Lover) && !x.Is(RoleEnum.Vampire) && !x.Data.Disconnected).ToList();
            ...
            var toChooseFrom = PlayerControl.AllPlayerControls.ToArray().Where(x => !x.Is(Faction.Crewmates) && !x.Is(Faction.Impostors) && !x.Is(ModifierEnum.Lover) && !x.Is(RoleEnum.Vampire) && x.Data.IsDead && !x.Data.Disconnected).ToList();
